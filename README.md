...
- Run `evaluation.py` to attack the target LLM using adversarial prompts and defend it with GradShield:
  ```shell
  python evaluation.py --model_name <name of target LLM> --Jailbreak <name of Jailbreak> [<name of Jailbreak> ...]
  ```
  **parameters**
  * `model_name`: The name of the target LLM, which is also the key in `model_path.json`, defaulting to `vicuna_7b_v1_5`
  * `Jailbreak`: One or more jailbreak attack names, corresponding to subdirectories in `/adversarial_prompts` directory, defaulting to `GCG`. For example: `--Jailbreak GCG PAIR` or `--Jailbreak "[GCG,PAIR]"`.
...
- Run `judgment.py` to judgment whether the final response is harmful:
  ```shell
  python judgment.py --model_name <name of target LLM> --Jailbreak <name of Jailbreak> [<name of Jailbreak> ...]
  ```
  **parameters**
  * `model_name`: The name of the target LLM, which is also the key in `model_path.json`
  * `Jailbreak`: One or more jailbreak attack names. For example: `--Jailbreak GCG PAIR` or `--Jailbreak "[GCG,PAIR]"`.
